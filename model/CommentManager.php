...
class CommentManager extends Manager{
	...
	public function getReportedTotalRows()
	{
		$db = $this->db();
		$totalRows = $db->query("SELECT *, DATE_FORMAT(creation_date, \" Le %d/%m/%Y à %Hh%imin%ss\") AS creation_date_fr FROM comments WHERE report_level > 0 ORDER BY id DESC");
		$totalRows = $totalRows->rowCount();
		return $totalRows;
	}

	public function getReportedCommsWithPagination($start, $commentsPerPage)
	{
		$db = $this->db();
